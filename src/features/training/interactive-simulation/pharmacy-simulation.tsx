...
export function PharmacySimulation() {
  ...
  return <section className="overflow-hidden rounded-[22px] border border-slate-300 bg-[#d9e4e1] shadow-[0_28px_70px_rgb(15_23_42/.2)]">
    <header className="bg-gradient-to-r from-[#031225] via-[#09253e] to-[#031225] px-5 py-3 text-white"><p className="text-base font-extrabold">SIMULACIÓN: DISPENSACIÓN SEGURA PARA <span className="text-teal-300">TENS</span></p></header>
    <nav aria-label="Progreso operativo" className="flex items-center gap-2 overflow-x-auto bg-white/95 px-5 py-3 text-sm font-bold text-slate-600">{["Identificar", "Verificar sistema", "Revisar recetas", "Doble chequeo", "Despachar"].map((stage, index) => <div className={`flex shrink-0 items-center gap-2 ${index === 2 ? "rounded-xl bg-slate-100 px-3 py-2 text-slate-900" : ""}`} key={stage}><span className={`flex h-7 w-7 items-center justify-center rounded-full ${index === 2 ? "bg-blue-700 text-white" : "border border-slate-300 bg-white text-slate-500"}`}>{index + 1}</span>{stage}{index < 4 ? <span className="ml-3 text-slate-400">→</span> : null}</div>)}</nav>
    <div className="block p-3">
      ...
      <main className="relative min-h-[680px] overflow-hidden rounded-2xl border border-slate-300 bg-slate-200 shadow-xl"><Image alt="Farmacia ficticia para entrenamiento" className="object-cover" fill priority sizes="(max-width: 1280px) 100vw, 55vw" src="/scenes/pharmacy-training-room-v3.png" /><div className="absolute inset-0 bg-gradient-to-t from-[#071c2a99] via-transparent to-transparent" /><div aria-label="TENS jugadora en ventanilla" className="absolute bottom-0 right-[19%] z-[9] h-48 w-24 rounded-t-full bg-[#0b6674] shadow-[-8px_0_0_rgb(4_43_54/.3)]"><span className="absolute -top-10 left-5 h-14 w-14 rounded-full bg-[#b87f63]" /><span className="absolute -top-3 left-1 h-10 w-20 rounded-t-full bg-[#172334]" /></div><button aria-label="Segunda TENS y bandeja" className="absolute right-[9%] top-[18%] z-10 flex flex-col items-center gap-1 rounded-xl bg-white/10 p-2 text-white hover:bg-white/25" onClick={() => setFocus("tens")} type="button"><span className="h-10 w-10 rounded-full bg-[#ae7058] ring-4 ring-[#08707c]" /><span className="h-16 w-14 rounded-t-2xl bg-[#08707c]" /><span className="text-[10px] font-bold">TENS 2</span></button><button className="absolute bottom-0 left-1/2 z-10 w-[48%] min-w-52 max-w-96 -translate-x-1/2 focus-visible:outline-4 focus-visible:outline-teal-300" onClick={() => setFocus("patient")} type="button"><Image alt="Paciente virtual Luis Herrera" className="h-auto w-full drop-shadow-[0_18px_14px_rgb(0_0_0/.3)]" draggable={false} height={1536} sizes="(max-width: 700px) 62vw, 24rem" src="/scenes/patient-a01-3d-render-v3.png" width={1024} /></button><div className="absolute right-4 top-5 z-20 max-w-48 rounded-xl border border-teal-300 bg-[#063a47]/90 p-3 text-white shadow-lg"><b className="text-sm">Documento de identidad</b><p className="mt-1 text-xs leading-5">Acércate al paciente para solicitarlo.</p></div><button aria-label="Revisar bandeja preparada" className="absolute bottom-10 right-5 z-20 rounded-xl border border-teal-200 bg-[#063a47]/95 p-3 text-left text-white shadow-lg" onClick={() => setFocus("tray")} type="button"><span className="flex items-center gap-2 text-sm font-bold"><Icon name="tray" /> Bandeja preparada</span><span className="mt-1 block text-xs text-teal-100">Inspeccionar contenido</span></button><button aria-label="Abrir sistema" className="absolute bottom-10 left-5 z-20 rounded-xl border border-white/60 bg-white/90 p-3 text-left text-slate-900 shadow-lg" onClick={() => setFocus("computer")} type="button"><span className="flex items-center gap-2 text-sm font-bold"><Icon name="computer" /> Sistema</span><span className="mt-1 block text-xs text-slate-600">Ficha y recetas</span></button><button aria-label="Iniciar entrega de medicamentos" className="absolute bottom-6 left-1/2 z-20 flex h-14 w-14 -translate-x-1/2 items-center justify-center rounded-full border-2 border-amber-200 bg-amber-50 text-amber-800 shadow-lg transition hover:scale-110 hover:bg-amber-100" onClick={() => setFocus("handoff")} type="button"><Icon name="handoff" /></button></main>
    </div>
