...
export function ContextualStorageExperience({ levelNumber, mode, trainingCase }: Props) {
  ...
  async function finish() {
    ...
    setSaveResult(await saveSimulationAttempt({
      ...
      <div className="grid xl:grid-cols-[minmax(0,1fr)_27.5rem]">
        <div className="min-w-0 border-b border-violet-100 xl:border-b-0 xl:border-r">
          <div className="relative min-h-[720px] overflow-hidden bg-[#e9e8ec]">
            <img alt="Área ficticia de almacenamiento de farmacia" className="absolute inset-0 h-full w-full scale-[1.08] object-cover object-[72%_43%]" draggable={false} src="/images/farmasim/case001-scene.png" />
            <div className="absolute inset-0 bg-slate-950/10" />
            {!finished ? <button aria-label="Revisar zona de almacenamiento" className="group absolute left-[72%] top-[25%] z-20 -translate-x-1/2 -translate-y-1/2" onClick={() => setSelected(true)} type="button"><span className="mx-auto block size-3.5 rounded-full border-[3px] border-white bg-violet-600/90 shadow-lg transition group-hover:scale-125" /><span className={mode.guidance === "guided" ? "mt-2 block rounded-xl bg-white/95 px-3 py-1.5 text-xs font-bold text-violet-700 shadow" : "mt-2 block translate-y-1 rounded-xl bg-white/95 px-3 py-1.5 text-xs font-bold text-violet-700 opacity-0 shadow transition group-hover:translate-y-0 group-hover:opacity-100"}>Zona de almacenamiento</span></button> : null}
