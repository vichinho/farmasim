...
export default async function CapsuleDetailPage({ params }: Props) {
  ...
  return (
    <main className="mx-auto w-full max-w-4xl px-4 pb-28 pt-5 sm:px-6 sm:pt-8 lg:px-8">
      <nav className="flex flex-wrap items-center justify-between gap-2" aria-label="Navegación de cápsula">
        <Link className="inline-flex min-h-10 items-center rounded-xl border border-violet-100 bg-white px-3 text-sm font-black text-violet-700 shadow-[0_5px_18px_rgba(76,48,130,.06)] transition hover:bg-violet-50" href="/capsulas">← Mis cápsulas</Link>
        <Link className="inline-flex min-h-10 items-center rounded-xl border border-slate-200 bg-white px-3 text-sm font-black text-slate-600 transition hover:bg-slate-50" href="/dashboard">Dashboard</Link>
      </nav>
      <article className="mt-3 overflow-hidden rounded-[1.5rem] border border-violet-100 bg-white sm:mt-4 sm:rounded-[2rem]">
        <header className="border-b border-violet-100 bg-violet-50/40 p-5 sm:p-8">
          ...
          <h1 className="mt-3 text-2xl font-black text-slate-950 sm:text-3xl">{capsule.title}</h1>
          {capsule.summary ? <p className="mt-2 text-sm leading-6 text-slate-600">{capsule.summary}</p> : null}
        </header>
        <div className="space-y-5 p-5 sm:p-8">
          {capsule.content ? <div className="whitespace-pre-wrap rounded-2xl bg-slate-50 p-4 text-sm leading-7 text-slate-700 sm:p-5">{capsule.content}</div> : null}
